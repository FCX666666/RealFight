...
function updateChildComponent(
  ...
  if (needsForceUpdate) { 
    // 当在作用域插槽的情况下 进行一次强制更新  触发子组件渲染  
    // 因为作用域插槽的情况下 数据是在子组件的上下文中（初始化）渲染的 所以只收集了子组件的渲染watcher 所以无法通过数据更新进而更新子组件
...
function matches(pattern, name) { // 根据用户传入的字符串或是正则表达式或是数组 分情况进行判断
